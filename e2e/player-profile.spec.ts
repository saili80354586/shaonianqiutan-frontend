...
test.describe('球员资料 API 验证', () => {
  ...
  test('登录并获取球员资料', async ({ request }) => {
    ...
    const profileRes = await request.get('http://localhost:8080/api/player/profile', {
  ...
  test('更新球员资料', async ({ request }) => {
    ...
    const updateRes = await request.put('http://localhost:8080/api/player/profile', {
      headers: { 
  ...
  test('球探地图返回真实数据', async ({ request }) => {
    const mapRes = await request.get('http://localhost:8080/api/scout/players/101/map-profile');
